...
class AnalysisAgent:
    ...
    def _analyze_taxes(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        carga_tributaria = (
            (total_impostos / total_produtos) * 100 if total_produtos > 0 else 0
        )
        ...
        return {
            ...
            "maior_imposto": (
                max(tax_breakdown.items(), key=lambda x: x[1]["valor"])[0]
                if tax_breakdown
                else None
            ),
        }
    ...
    def _analyze_items(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        item_mais_caro = (
            max(itens, key=lambda x: float(x.get("valor_total", 0))) if itens else None
        )
        item_maior_quantidade = (
            max(itens, key=lambda x: float(x.get("quantidade", 0))) if itens else None
        )
        ...
        return {
            ...
            "item_mais_caro": {
                "descricao": (
                    item_mais_caro.get("descricao", "N/A") if item_mais_caro else None
                ),
                "valor": item_mais_caro.get("valor_total", 0) if item_mais_caro else 0,
            },
            "item_maior_quantidade": {
                "descricao": (
                    item_maior_quantidade.get("descricao", "N/A")
                    if item_maior_quantidade
                    else None
                ),
                "quantidade": (
                    item_maior_quantidade.get("quantidade", 0)
                    if item_maior_quantidade
                    else 0
                ),
            },
    ...
    def _financial_summary(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "margem_liquida_percent": (
                ((valor_total - impostos) / valor_total * 100) if valor_total > 0 else 0
            ),
        }
    ...
    def _check_compliance(self, document_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "num_issues": len(validation.get("errors", []))
    ...
    def _generate_recommendations(self, data: Dict[str, Any]) -> List[str]:
        ...
        total_impostos = sum(
            float(impostos.get(k, 0)) for k in ["icms", "ipi", "pis", "cofins"]
        )
        ...
        if total_produtos > 0:
            ...
            if carga_tributaria > 30:
                recommendations.append(
                    "⚠️ Carga tributária acima de 30% - Considere revisar o regime tributário"
                )

            if carga_tributaria > 40:
                recommendations.append(
                    "🔴 Carga tributária muito alta (>40%) - Recomenda-se consultoria fiscal"
                )

        desconto = float(totais.get("desconto", 0))
        if desconto > total_produtos * 0.15:
            recommendations.append(
                "💡 Desconto significativo aplicado (>15%) - Verifique margem de lucro"
            )

        itens = data.get("itens", [])
        if len(itens) > 50:
            recommendations.append(
                "📊 Nota fiscal com muitos itens - Considere uso de sistema ERP para gestão"
            )
    ...
    def analyze_multiple_documents(
        ...
        document_types = Counter(
            doc.get("document_type") for doc in documents if doc.get("document_type")
        )

        issuers = Counter(
            doc.get("issuer_name") for doc in documents if doc.get("issuer_name")
        )
        ...
        return {
            "overview": {
                ...
                "average_value": (
                    total_value / total_documents if total_documents > 0 else 0
                ),
                "tax_burden_percent": (
                    (total_taxes / total_value * 100) if total_value > 0 else 0
                ),
            },
            "by_type": dict(document_types),
            "top_issuers": [
                {"name": name, "count": count} for name, count in top_issuers
            ],
    ...
    def _group_by_month(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for doc in documents:
            ...
            if created_at:
                ...
                monthly[month_key]["total_value"] += float(
                    doc.get("total_value", 0) or 0
                )
    ...
    def _generate_aggregate_insights(
        ...
        if document_types:
            ...
            insights.append(
                f"📄 Tipo mais comum: {most_common[0]} ({most_common[1]} documentos)"
            )
