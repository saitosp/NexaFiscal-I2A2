...
class CriticAgent:
    ...
    def __init__(self):
        ...
        if not self.is_available:
            print(
                "⚠️ GROQ_API_KEY não configurada. Crítico funcionará em modo degradado."
            )
            self.client = None
            self.model = None
        else:
    ...
    def review_output(
        ...
        if not self.is_available or not self.client:
            return {
                ...
                "recommendations": [
                    "Configure GROQ_API_KEY para análise crítica completa"
                ],
                "confidence": 0.5,
...
"""
    ...
    def improve_response(
        self, original_response: str, review: Dict[str, Any], user_question: str
    ) -> str:
        """
