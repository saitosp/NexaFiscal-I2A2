...
class ClassificationAgent:
    ...
    def __init__(self):
        ...
        if not api_key:
            raise ValueError(
                "GROQ_API_KEY não configurada. Configure a chave de API do Groq nas variáveis de ambiente."
            )
...
Responda APENAS com o tipo do documento (NFe, NFCe, SAT, CTe, NFSe, Cupom Fiscal ou Outro)."""
            ...
            completion = self.client.chat.completions.create(
                model=self.model, messages=messages, max_tokens=50, temperature=0.3
            )
