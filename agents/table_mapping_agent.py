...
class TableMappingAgent:
    ...
    def __init__(self):
        ...
        if not self.is_available:
            print(
                "⚠️ GROQ_API_KEY não configurada. Usando mapeamento básico por padrões."
            )
            self.client = None
            self.model = None
        else:
    ...
    def auto_map_columns(
        self, columns: List[str], sample_data: List[Dict[str, Any]] = None
    ) -> Dict[str, str]:
        """
...
Retorne o mapeamento em JSON:"""
    ...
    def validate_mapping(
        self, mapping: Dict[str, str], required_fields: List[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def suggest_improvements(
        self, mapping: Dict[str, str], columns: List[str]
    ) -> List[str]:
        """
