...
def render_sefaz_integration():
    ...
    st.write(
        "Configure certificado digital e sincronize documentos fiscais dos portais oficiais"
    )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📜 Certificados", "🔄 Sincronização", "ℹ️ Informações"]
    )
        ...
        with st.expander("➕ Adicionar Novo Certificado", expanded=False):
            ...
            cert_environment = st.selectbox(
                ...
                format_func=lambda x: (
                    "Homologação" if x == "homologation" else "Produção"
                ),
            )

            if st.button(
                "📤 Enviar Certificado", use_container_width=True, type="primary"
            ):
                if not cert_name:
                    ...
                    with st.spinner("Processando certificado..."):
                        try:
                            ...
                            if response.status_code == 200:
                                data = response.json()
                                st.success(
                                    f"✅ Certificado adicionado com sucesso! (ID: {data['credential_id']})"
                                )
                                ...
                                **Informações do Certificado:**
                                ...
                                st.error(
                                    f"❌ Erro ao processar certificado: {error_detail}"
                                )
            ...
            if response.status_code == 200:
                ...
                if certificates:
                    for cert in certificates:
                        ...
                        env_icon = (
                            "🔴" if cert.get("environment") == "production" else "🟡"
                        )
                        ...
                        with st.expander(
                            ...
                            with col1:
                                ...
                                st.write(
                                    f"**Ambiente:** {cert.get('environment', 'N/A').title()}"
                                )
                                st.write(
                                    f"**Status:** {'Ativo' if cert.get('is_active') else 'Inativo'}"
                                )

                            with col2:
                                st.write(
                                    f"**Subject:** {cert.get('subject', 'N/A')[:50]}..."
                                )
                                st.write(
                                    f"**Válido Até:** {cert.get('valid_until', 'N/A')[:19]}"
                                )
                                st.write(
                                    f"**Cadastrado em:** {cert.get('created_at', 'N/A')[:19]}"
                                )
                            ...
                            with col_test:
                                ...
                                if st.button(
                                    "🔍 Testar Certificado", key=f"test_{cert['id']}"
                                ):
                                    if test_password:
                                        try:
                                            ...
                                            if test_response.status_code == 200:
                                                ...
                                                if test_result["valid"]:
                                                    ...
                                                    if test_result.get("details"):
                                                        ...
                                                        st.info(
                                                            f"Dias até expiração: {details.get('days_until_expiry', 'N/A')}"
                                                        )
                                                else:
                    ...
                    st.info(
                        "📭 Nenhum certificado cadastrado. Adicione um certificado acima para começar."
                    )
            else:
                ...
                if certificates:
                    ...
                    if cert_options:
                        selected_cert_label = st.selectbox(
                            "Selecione o Certificado", options=list(cert_options.keys())
                        )

                        selected_cert_id = cert_options[selected_cert_label]

                        sync_password = st.text_input(
                            "Senha do Certificado", type="password", key="sync_password"
                        )
                                ...
                                with st.spinner("Consultando SEFAZ..."):
                                    try:
                                        ...
                                        if sync_response.status_code == 200:
                                            ...
                                            st.error(
                                                f"Erro na sincronização: {sync_response.text}"
                                            )
                        ...
                        st.warning(
                            "⚠️ Nenhum certificado ativo disponível. Adicione um certificado na aba 'Certificados'."
                        )
                else:
                    st.info(
                        "📭 Nenhum certificado cadastrado. Adicione um certificado na aba 'Certificados' primeiro."
                    )
