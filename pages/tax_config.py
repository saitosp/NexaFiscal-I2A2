...
def render_tax_list(tax_config):
    ...
    with col1:
        ...
        selected_toggle = st.selectbox(
            "Selecione o imposto:", options=tax_ids, key="toggle_select"
        )
    ...
    with col2:
        ...
        selected_edit = st.selectbox(
            "Selecione o imposto:", options=tax_ids, key="edit_select"
        )
    ...
    with col3:
        ...
        selected_delete = st.selectbox(
            "Selecione o imposto:", options=tax_ids, key="delete_select"
        )
...
def render_add_tax_form(tax_config):
    ...
    with st.form("add_tax_form"):
        ...
        tax_name = st.text_input(
            "Nome*", placeholder="ex: IVA", help="Nome curto do imposto"
        )
...
def render_edit_tax_form(tax_config, tax_id):
    ...
    with st.form("edit_tax_form"):
        tax_name = st.text_input(
            "Nome*", value=current_tax.get("name", ""), help="Nome curto do imposto"
        )
        ...
        tax_scope = st.selectbox(
            "Escopo*",
            options=["federal", "estadual", "municipal"],
            index=["federal", "estadual", "municipal"].index(
                current_tax.get("scope", "federal")
            ),
            help="Jurisdição do imposto",
        )
        ...
        tax_xml_fields = st.text_input(
            "Campos XML*", value=xml_fields_str, help="Campos XML separados por vírgula"
        )
