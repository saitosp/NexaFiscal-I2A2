...
def render_batch_processing():
    ...
    st.write(
        "Faça upload de múltiplos documentos fiscais para processamento simultâneo"
    )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📤 Novo Lote", "📊 Lotes Ativos", "📚 Histórico de Lotes"]
    )
    ...
    with tab1:
        ...
        if uploaded_files:
            ...
            with col1:
                if st.button(
                    "🚀 Enviar e Processar", use_container_width=True, type="primary"
                ):
                    with st.spinner("Enviando arquivos..."):
                        try:
                            ...
                            response = requests.post(
                                f"{API_URL}/api/batch/upload", files=files
                            )
                            ...
                            if response.status_code == 200:
                                ...
                                st.success(
                                    f"✅ {data['total_files']} arquivos enviados com sucesso!"
                                )
                                ...
                                if process_response.status_code == 200:
                                    ...
                                    st.warning(
                                        "Arquivos enviados, mas processamento não iniciado automaticamente"
                                    )
                            else:
        ...
        if "current_batch_id" in st.session_state:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Barra de progresso
                    if status["total"] > 0:
                        progress = (status["completed"] + status["failed"]) / status[
                            "total"
                        ]
                    ...
                    if status["items"]:
                        ...
                        if status["failed"] > 0:
                            if st.button(
                                f"🔁 Reprocessar {status['failed']} Documentos com Falha"
                            ):
                                retry_response = requests.post(
                                    f"{API_URL}/api/batch/{batch_id}/retry"
                                )
                                if retry_response.status_code == 200:
                                    st.success(
                                        f"✅ {retry_response.json()['retried_count']} documentos resetados"
                                    )
                                    time.sleep(1)
                                    st.rerun()
            ...
            st.info(
                "📭 Nenhum lote ativo. Faça upload de documentos na aba 'Novo Lote'"
            )
                    ...
                    for batch in batches:
                        with st.expander(
                            ...
                            with col1:
                                ...
                                st.write(
                                    f"**Data:** {batch.get('created_at', 'N/A')[:19] if batch.get('created_at') else 'N/A'}"
                                )
                            ...
                            if st.button(
                                "Ver Detalhes", key=f"view_{batch['batch_id']}"
                            ):
