...
from utils.tax_config_loader import (
    get_tax_config,
    get_all_tax_names,
    get_all_tax_colors,
)
...
def render_dashboard():
    ...
    st.markdown(
        "**Visualizações interativas e insights automáticos dos documentos processados**"
    )
    ...
    try:
        dashboard_data = AnalysisService.get_dashboard_data(
            batch_ids=selected_batch_ids if selected_batch_ids is not None else []
        )
        ...
        if "error" in dashboard_data:
            ...
            if dashboard_data.get("overview", {}).get("total_documents", 0) == 0:
                st.info(
                    "📭 Nenhum documento processado ainda. Faça upload de notas fiscais para começar!"
                )
                return
...
def render_batch_selector():
    ...
    if mode == "📊 Todos os documentos":
        ...
        batch_options = {
            f"{batch['batch_name']} ({batch['document_count']} docs, R$ {batch['total_value']:,.2f})": batch[
                "batch_id"
            ]
        ...
        selected_batch = next(
            b for b in all_batches if b["batch_id"] == selected_batch_id
        )
        ...
        with col3:
            ...
            st.metric(
                "📅 Criado em",
                created_date.strftime("%d/%m/%Y") if created_date else "N/A",
            )
        ...
        total_docs = sum(
            ...
            if b["batch_id"] in selected_batch_ids
        )
        total_value = sum(
            b["total_value"] for b in all_batches if b["batch_id"] in selected_batch_ids
        )
...
def render_document_types_chart(by_type):
    ...
    df = pd.DataFrame(
        [{"Tipo": tipo, "Quantidade": qtd} for tipo, qtd in by_type.items()]
    )
...
def render_tax_breakdown_chart(batch_ids=None):
    ...
    tax_data = AnalysisService.get_tax_analysis(
        batch_ids=batch_ids if batch_ids is not None else []
    )
    ...
    for tax in tax_config.get_all_taxes(enabled_only=True):
        ...
        if tax_value > 0:
            tax_rows.append({"Imposto": tax_name, "Valor": tax_value, "tax_id": tax_id})
    ...
    color_map = {
        row["Imposto"]: tax_colors.get(row["tax_id"], "#808080") for row in tax_rows
    }
    ...
    fig.update_layout(showlegend=False, height=400, yaxis_title="Valor (R$)")
...
def render_top_products_table(batch_ids=None):
    ...
    products = AnalysisService.get_top_products(
        limit=10, batch_ids=batch_ids if batch_ids is not None else []
    )
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Quantidade Total": st.column_config.TextColumn(
                "Quantidade", width="medium"
            ),
        },
    )
