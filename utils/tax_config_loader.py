...
class TaxConfig:
    ...
    def get_tax_names(self, enabled_only: bool = True) -> Dict[str, str]:
        ...
        """
        return {tax["id"]: tax["name"] for tax in self.get_all_taxes(enabled_only)}
    ...
    def get_taxes_for_document_type(self, doc_type: str) -> List[Dict[str, Any]]:
        ...
        """
        return [
            tax for tax in self.get_all_taxes() if doc_type in tax.get("applies_to", [])
        ]
    ...
    def _add_change_to_history(
        self, change_description: str, author: str = "user"
    ) -> None:
        """
        ...
        for i, tax in enumerate(self._config["taxes"]):
            if tax["id"] == tax_id:
                ...
                self._add_change_to_history(
                    f"Imposto '{updated_data['name']}' atualizado"
                )
                self._save_config()
                self.reload()
                get_tax_config.cache_clear()
