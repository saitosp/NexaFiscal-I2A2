...
def convert_to_fiscal_documents(
    ...
    for row_idx_enum, (idx, row) in enumerate(df.iterrows()):
        ...
        if "emitente_cnpj" in column_mapping and column_mapping["emitente_cnpj"]:
            ...
            doc["emitente"]["cnpj"] = (
                str(row[cnpj_col]).replace(".", "").replace("-", "").replace("/", "")
            )

        # Destinatário
        if (
            "destinatario_nome" in column_mapping
            and column_mapping["destinatario_nome"]
        ):
            doc["destinatario"]["nome"] = str(row[column_mapping["destinatario_nome"]])
        if (
            "destinatario_cnpj" in column_mapping
            and column_mapping["destinatario_cnpj"]
        ):
        ...
        if "valor_total" in cleaned_cols and not pd.isna(
            cleaned_cols["valor_total"].iloc[row_idx_enum]
        ):
            doc["totais"]["valor_total"] = float(
                cleaned_cols["valor_total"].iloc[row_idx_enum]
            )
        else:
            doc["totais"]["valor_total"] = 0.0

        if "valor_produtos" in cleaned_cols and not pd.isna(
            cleaned_cols["valor_produtos"].iloc[row_idx_enum]
        ):
            doc["totais"]["valor_produtos"] = float(
                cleaned_cols["valor_produtos"].iloc[row_idx_enum]
            )
        else:
        ...
        if "icms" in cleaned_cols and not pd.isna(
            cleaned_cols["icms"].iloc[row_idx_enum]
        ):
            doc["impostos"]["icms"] = float(cleaned_cols["icms"].iloc[row_idx_enum])
        else:
            doc["impostos"]["icms"] = 0.0

        if "pis" in cleaned_cols and not pd.isna(
            cleaned_cols["pis"].iloc[row_idx_enum]
        ):
            doc["impostos"]["pis"] = float(cleaned_cols["pis"].iloc[row_idx_enum])
        else:
            doc["impostos"]["pis"] = 0.0

        if "cofins" in cleaned_cols and not pd.isna(
            cleaned_cols["cofins"].iloc[row_idx_enum]
        ):
            doc["impostos"]["cofins"] = float(cleaned_cols["cofins"].iloc[row_idx_enum])
        else:
            doc["impostos"]["cofins"] = 0.0

        if "ipi" in cleaned_cols and not pd.isna(
            cleaned_cols["ipi"].iloc[row_idx_enum]
        ):
