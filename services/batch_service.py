...
class BatchService:
    ...
    def create_batch(files_data: List[Dict[str, Any]]) -> str:
        ...
        for file_info in files_data:
            ProcessingQueueRepository.create(
                batch_id=batch_id,
    ...
    def mark_processing(queue_id: int) -> None:
        ...
        ProcessingQueueRepository.update_status(
            queue_id, "processing", started_at=datetime.now()
        )
    ...
    def mark_completed(queue_id: int, document_id: int) -> None:
        ...
        """
        ProcessingQueueRepository.update_status(
            queue_id, "completed", document_id=document_id, completed_at=datetime.now()
        )
    ...
    def mark_failed(queue_id: int, error_message: str) -> None:
        ...
        """
        ProcessingQueueRepository.update_status(
            queue_id, "failed", error_message=error_message, completed_at=datetime.now()
        )
    ...
    def retry_failed(batch_id: str) -> int:
        ...
        for item in items:
            if item.status == "failed" and item.attempts < 3:
                ProcessingQueueRepository.update_status(
                    item.id, "pending", error_message=None
                )
                count += 1
    ...
    def get_all_batches(limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for item in all_items:
            ...
            if (
                item.created_at
                and item.created_at < batches_dict[batch_id]["created_at"]
            ):
    ...
    def get_batch_summary(batch_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "updated_at": (
                    batch.updated_at.isoformat() if batch.updated_at else None
                ),
            }
        finally:
            session.close()
