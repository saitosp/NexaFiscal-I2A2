...
class SefazService:
        ...
        try:
            ...
            private_key, certificate, additional_certs = (
                pkcs12.load_key_and_certificates(
                    certificate_file, password.encode(), backend=default_backend()
                )
            )
    ...
    def get_certificate_data(credential_id: int, password: str) -> Tuple[bytes, bytes]:
        ...
        if (
            ...
            and datetime.now(credential.expires_at.tzinfo) > credential.expires_at
        ):
            raise ValueError("Certificado expirado")
    ...
    def list_certificates() -> list:
        ...
        """
        credentials = CredentialRepository.get_all(
            credential_type="sefaz_certificate_a1"
        )
            ...
            result.append(
                {
                    ...
                    "valid_from": (
                        cred.metadata.get("valid_from") if cred.metadata else None
                    ),
                    "valid_until": (
                        cred.metadata.get("valid_until") if cred.metadata else None
                    ),
                    "subject": cred.metadata.get("subject") if cred.metadata else None,
                    "created_at": (
                        cred.created_at.isoformat() if cred.created_at else None
                    ),
                }
            )
    ...
    def test_certificate(credential_id: int, password: str) -> Dict[str, Any]:
        ...
        """
        try:
            cert_pem, key_pem = SefazService.get_certificate_data(
