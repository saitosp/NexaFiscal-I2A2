...
class DocumentService:
    ...
    def save_processed_document(result: Dict[str, Any]) -> int:
        ...
        try:
            ...
            document_data = {
                ...
                "total_value": (
                    float(totais.get("valor_total", 0))
                    if totais.get("valor_total")
                    else None
                ),
                "tax_total": (
                    float(totais.get("total_impostos", 0))
                    if totais.get("total_impostos")
                    else None
                ),
                ...
                or len(validation.get("errors", [])) > 0,
                "processing_status": (
    ...
    def search_documents(
        query: str, document_type: Optional[str] = None
    ) -> List[Document]:
        """
    ...
    def document_to_result_format(doc: Document) -> Dict[str, Any]:
        ...
        return {
            ...
            "classification": (
                doc.classification_data if doc.classification_data is not None else {}
            ),
            "extracted_data": (
                doc.extracted_data if doc.extracted_data is not None else {}
            ),
            "validation": (
                doc.validation_data if doc.validation_data is not None else {}
            ),
            "errors": (
                [] if not bool(doc.has_errors) else ["Documento processado com erros"]
            ),
