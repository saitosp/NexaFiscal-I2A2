...
class AnalysisService:
    ...
    def analyze_document(document_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "error": str(e),
                "note": "Configure GROQ_API_KEY para habilitar análise com IA",
            }
    ...
    def get_top_products(
        limit: int = 10, batch_ids: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
            ...
            if doc.extracted_data:
                ...
                for item in itens:
                    ...
                    if descricao:
                        ...
                        product_counter[descricao]["total_value"] += float(
                            item.get("valor_total", 0)
                        )
                        product_counter[descricao]["total_quantity"] += float(
                            item.get("quantidade", 0)
                        )

        sorted_products = sorted(
            product_counter.values(), key=lambda x: x["count"], reverse=True
        )
