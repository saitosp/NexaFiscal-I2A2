...
class ChatService:
    ...
    @staticmethod
    def create_session(
        db: Session, user_id: str = "default", title: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def process_message(
        ...
        user_msg = ChatMessage(
            session_id=session.id, role="user", content=user_message, meta_data={}
        )
        db.add(user_msg)
        db.commit()
        ...
        try:
            ...
            return {"role": "assistant", "content": error_msg.content, "error": str(e)}

    @staticmethod
    def list_sessions(
        db: Session, user_id: str = "default", limit: int = 20
    ) -> List[Dict[str, Any]]:
        """
